...
function loadSubscriptionData(showFeeds) {
	var defaultShowsObject;
	for(var i = 0; i < showFeeds.length; i++) {
		defaultShowsObject = getRSS(showFeeds[i].url);
		 if(i+1 == showFeeds.length) {
		 	//defaultShowsObject shows full array of show objects
		 	console.log(defaultShowsObject);
		 	//defaultShowsObject is empty after being ported to content script file?
			settings.port.emit("addSubscriptions", defaultShowsObject);
		 }
